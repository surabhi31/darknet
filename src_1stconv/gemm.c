...
void gemm_nn_hw(int M, int N, int K, float ALPHA, 
        int lda, 
        float B[array_size(27,27)], int ldb,
        float C[array_size(16,27)], int ldc,
        float D[array_size(16,27)], int N_K, int NN);
        ...
        float C[FIXED_SIZE], int ldc)
{
    ...
    float *IFMAP;
    IFMAP=(float *) malloc(K*K*sizeof(float));

    float *OFMAP;
    OFMAP=(float *) malloc(M*K*sizeof(float));

    float *OFMAP_TMP;
    OFMAP_TMP=(float *) malloc(M*K*sizeof(float));
    ...
    for(i = 0; i<K; ++i ) {
      for(j = 0; j<K; ++j ) {
        IFMAP[i*K+j] = B[i*ldb+j]; 
      }
    }
    printf("Allocating memory for IFMAP! \n");
    for(i = 0; i<M; ++i ) {
      for(j = 0; j<K; ++j ) {
      //  printf("i*K+j = %d, i*ldc+j = %d\n", i*K+j,i*ldc+j);
        OFMAP_TMP[i*K+j] = C[i*ldc+j]; 
      }
    }
    printf("Allocating memory for OFMAP! \n");


    printf("Entering compute in hw mode! \n");
    //gemm_nn_hw(M, K, K, ALPHA,lda, IFMAP, ldb,OFMAP_TMP,ldc,OFMAP,0,N);
    gemm_nn_hw(M, K, K, ALPHA,lda, IFMAP, K,OFMAP_TMP,K,OFMAP,0,N);
    printf("Exiting compute in hw mode! \n");
    for(i = 0; i<M; ++i ) {
      for(j = 0; j<K; ++j ) {
        C[i*ldc+j] = OFMAP[i*K+j];
      }
    }
    ...
    for(j = 1; j < N; j=j+K){
      gemm_nn_sw(M, K, K, ALPHA,lda, B, ldb,C,ldc,D,j,N);
    }
    C = D;
}
