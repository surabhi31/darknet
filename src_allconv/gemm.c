...
#define NUM1 16200
#define NUM2  9600
#pragma SDS data data_mover(B:AXIDMA_SIMPLE, C:AXIDMA_SIMPLE, D:AXIDMA_SIMPLE)
#pragma SDS data dim (B[NUM1],   C[NUM2],   D[NUM2])
#pragma SDS data copy(B[0:NUM1], C[0:NUM2], D[0:NUM2])
#pragma SDS data access_pattern(B:SEQUENTIAL, C:SEQUENTIAL, D:SEQUENTIAL)
void gemm_nn_hw(float ALPHA, 
        float B[array_size(27,600)], 
        float C[array_size(16,600)], 
        float D[array_size(16,600)] );
...
void gemm_nn_hw(float ALPHA, 
        ...
        float D[array_size(16,600)])
{
    int M = 16, N = 600, K = 27, ldc = 600, ldb = 600, lda = 27;
    int i,j,k;
    float Weights[432];
    ...
    L2: for(i = 0; i < M; ++i){
        L2_1: for(k = 0; k < K; ++k){
          //  register float A_PART = ALPHA*Weights[i*lda+k];
            L2_1_1: for(j = 0; j < N; ++j){
//            #pragma HLS unroll
                C[i*ldc+j] = Weights[i*lda+k] + B[k*ldb+j];
            }
        }
    }
        ...
        float C[FIXED_SIZE], int ldc)
{
    ...
    float *IFMAP;
    //IFMAP=(float *) sds_alloc(K*N*sizeof(float));
...
//    float *WEIGHTS;
//    WEIGHTS=(float *) sds_alloc(M*K*sizeof(float));

//    if (!IFMAP || !OFMAP || !OFMAP_TMP || !WEIGHTS) {
...
//    C = D;
    ...
    for(x = 0; x < N; x = x+P) {
      ...
      gemm_nn_hw( ALPHA,IFMAP, OFMAP,OFMAP_TMP);
